# Support And Sponsorship

This project is maintained as an independent public-good experiment.

The current sponsorship surface is intentionally conservative: this page is the
public support entry, and formal payment channels will be added only after the
owner enables and verifies an account-controlled funding path.

## Current status

- GitHub Sponsors or other direct payment channels are not claimed as active
  here unless the owner has enabled and verified them.
- The repository's GitHub funding button points to this page so visitors have a
  stable support entry without being sent to an unverified payment surface.
- Sponsorship does not buy approval, ranking, inclusion, or exemption from
  license, provenance, safety, privacy, or review gates.

## Useful support right now

If this project is useful, the best low-friction support is:

- star the repository;
- share a concrete use case;
- suggest a clearer project name while the current name is temporary;
- open a focused public-safe issue;
- contribute documentation, taxonomy, validation, or examples;
- test the public bookmark projection or resource-radar template and report
  what is confusing.

## Sponsorship interest

For sponsorship, funding, partnership, or long-form collaboration interest,
contact the maintainer through the public-safe contact routes in
[`docs/contact-and-social.md`](contact-and-social.md).

Do not send secrets, private configuration, private bookmarks, credentials,
local paths, account state, browser/session data, or private project material
through public issues, comments, screenshots, or social posts.

## What funding would support

Funding would help with:

- public-safe declassification work;
- documentation and first-time-user improvements;
- taxonomy and validation maintenance;
- generated artifact checks;
- example datasets and public-safe fixtures;
- GitHub Actions and future automation costs;
- review bandwidth for community suggestions;
- launch assets, tutorials, and short explainer videos.

## Future funding activation gate

Before adding a direct donation or sponsorship link, the repository should:

1. choose an owner-controlled funding platform;
2. verify the public URL works while logged out;
3. update `.github/FUNDING.yml`;
4. update README sustainability copy;
5. keep the no-pay-to-approve policy visible;
6. run repository verification;
7. record the change in a normal commit.

This keeps support useful without turning funding into a trust-boundary bypass.
